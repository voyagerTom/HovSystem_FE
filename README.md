...
測試Docker架設前後端分離場景，不小心踩了一個坑，前端在call BE時，使用BE容器IP為172.17.0.3:8099直接呼叫，衍生了些問題。
...
A1: 當user點擊UI時，所發起請求來源是user端，故若後端API設為 http://172.17.0.3:8099/user/...， 即表示 192.168.2.1直接呼叫172.17.0.3的資源，這導致user端找不到BE資源，因為172.17.0.3容器內的IP，只有再容器內同網段，才能存取成功。
